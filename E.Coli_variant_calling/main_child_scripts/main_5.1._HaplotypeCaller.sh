...
for s in $(cat $bam_files/bam_files.txt)
    do
        ...
        sbatch -J $samplename.HaplotypeCaller  ./E.Coli_variant_calling/main_child_scripts/child_5.1._HaplotypeCaller.sh $analysis_dir $bam_files $filename_extention $samplename
    done
