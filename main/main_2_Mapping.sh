...
for s in $(cat $analysis_dir/samplenames.txt)
    do
        ...
        sbatch -J $samplename.2_mapping $analysis_dir/child/child_2_Mapping.sh $analysis_dir $processed_files $paired_end $filename_extention $samplename $readname $genomeDir $reference_genome38 $reference_gtf $threads $overhang
    done
